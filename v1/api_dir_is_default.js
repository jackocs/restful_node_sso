	...
	config.host = stdout.trim();
		...
		if (error) {
			...
			//connection.query("select COUNT(*) as count from oauth_directory where orders='"+ ord +"'", function (error, results, fields) { 
	    		connection.query("select id from oauth_directory where is_default='1'", function (error, results, fields) {
			if (error) {
                    		result = {'status':'fail','result': error.message};                                                                                         
                    		return res.json(result);                                                                                                                    
                	}else{
				...
				var sql = "UPDATE oauth_directory SET `is_default` ='0' WHERE `id` ='"+ results[0].id +"'";
                                connection.query(sql, function (error, results, fields) {
					if (error) {
                    				result = {'status':'fail','result': error.message};
                    				return res.json(result);
					}else{
						var sql = "UPDATE oauth_directory SET `is_default` ='1' WHERE `id` ='"+ id  +"'";
		                                connection.query(sql, function (error, results, fields) {
							if (error) {
                    						result = {'status':'fail','result': error.message};
                    						return res.json(result);
							}else{
								exec("php /home/xIDM-SSO-Cent8/sso/idp/config/mysql2redis_local.php oauth_directory edit ", function (error, stdout, stderr) {
                                                        		if (error !== null) {
                                                                		result = {'status':'fail','result': error};
                                                                		return res.json(result);
                                                        		}else{
