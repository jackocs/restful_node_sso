...
router.get('/:group_policy_id', function (req, res) {
	...
	child = exec("docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' db ", function (error, stdout, stderr) {
	    ...
	    connection.query("select * from oauth_group_policy where group_policy_id='"+ group_policy_id +"'", function (error, results, fields) {
                if (error) {                                                                                          
                    ...
                    return res.json(result);
            			...
            			connection.query("DELETE FROM oauth_group_policy where group_policy_id='"+ group_policy_id +"'", function (error, results, fields) {
                			if (error) {	
