...
///                    return res.json(result);                                                                                                                    
            			...
            			connection.query("DELETE FROM oauth_scopes where scope='"+ scope +"'", function (error, results, fields) {
                			if (error) {	
						let result = {'status':'fail','result': error.message};
						return res.json(result); 
					}else{
						exec("php /home/xIDM-SSO-Cent8/sso/idp/config/mysql2redis_local.php oauth_scopes delete "+ scope, function (error, stdout, stderr) {
                                                        if (error !== null) {
                                                                result = {'status':'fail','result': error};
                                                                return res.json(result);
                                                        }else{
