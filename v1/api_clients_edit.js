...
router.post('', function(req, res) {
    if (req.body.client_name === undefined || req.body.client_name === null || 
        req.body.client_id === undefined || req.body.client_id === null ||
	...
	config.host = stdout.trim();
	    ...
	    connection.query("select COUNT(*) as count from oauth_clients where client_id='"+ client_id +"'", function (error, results, fields) {                                                      
		if (error) {
		    ...
		    if(results[0].count === 0){
				...
				var sql = "UPDATE oauth_clients set client_name=?,client_details=?,redirect_uri=?,scope=?,group_policy_id=? WHERE client_id=?";
	    	        	//var values = [[client_name, client_details, redirect_uri, scope, group_policy_id, client_id],];
				connection.query(sql, [client_name, client_details, redirect_uri, scope, group_policy_id, client_id], function (error, results, fields) {
					if (error) {
                    				result = {'status':'fail','result': error.message};
                    				return res.json(result);
					}else{
