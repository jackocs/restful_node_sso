    ...
    child = exec(
      ...
      function (error, stdout, stderr) {
        ...
        connection.connect(function (err) {
          ...
          connection.query(
            "select orders from oauth_directory where id='" + id + "'",
            function (error, results, fields) {
              if (error) {
                ...
                if (results[0].orders === 0) {
                  ...
                  connection.query(
                    "select id from oauth_directory where orders='" + ord + "'",
                    function (error, results, fields) {
                      if (error) {
                        result = { status: "fail", result: error.message };
                        return res.json(result);
                        ...
                        if (results[0].id > 0) {
                          ...
                          var sql =
                            "UPDATE oauth_directory SET `orders` ='99999' WHERE `id` ='" +
                            results[0].id +
                            "'";
                          connection.query(
                            sql,
                            function (error, results, fields) {
                              if (error) {
                                ...
                                var sql =
                                  "UPDATE oauth_directory SET `orders` ='" +
                                  ord +
                                  "' WHERE `id` ='" +
                                  id +
                                  "'";
