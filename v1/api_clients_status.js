...
router.get('/:id/:statuss', function (req, res) {
	...
	config.host = stdout.trim();
	    ...
	    connection.query("select COUNT(*) as count from oauth_clients where client_id='"+ id +"'", function (error, results, fields) {                                                      
		if (error) {
		    ...
		    if(results[0].count === 0){
				...
				var sql = "UPDATE oauth_clients SET `client_use` ='"+ sta +"' WHERE `client_id` ='"+ id  +"'";
				var values = [[sta,id],];
                                connection.query(sql, [sta,values], function (error, results, fields) {
					if (error) {
                    				result = {'status':'fail','result': error.message};
                    				return res.json(result);
					}else{
						exec("php /home/xIDM-SSO-Cent8/sso/idp/config/mysql2redis_local.php oauth_clients edit "+ id, function (error, stdout, stderr) {
                                                        if (error !== null) {
                                                                result = {'status':'fail','result': error};
                                                                return res.json(result);
                                                        }else{
