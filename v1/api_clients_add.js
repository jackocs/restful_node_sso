    ...
    child = exec(
      ...
      function (error, stdout, stderr) {
        ...
        connection.connect(function (err) {
          ...
          connection.query(
            "select COUNT(*) as count from oauth_clients where client_name='" +
              client_name +
              "'",
            function (error, results, fields) {
              if (error) {
                ...
                if (results[0].count > 0) {
                  ...
                  var sql =
                    "INSERT INTO oauth_clients (client_id,client_name,client_details,client_secret,redirect_uri,grant_types,scope,group_policy_id,client_use) VALUES ?";
                  var values = [
                    [
                      client_id,
                      client_name,
                  ...
                  connection.query(
                    ...
                    function (error, results, fields) {
                      if (error) {
                        ...
                        exec(
                          "php /home/xIDM-SSO-Cent8/sso/idp/config/mysql2redis_local.php oauth_clients add " +
                            client_id,
                          function (error, stdout, stderr) {
                            if (error !== null) {
                              result = { status: "fail", result: error };
