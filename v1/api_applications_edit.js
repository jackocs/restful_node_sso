	...
	config.host = stdout.trim();
		...
		if (error) {
		    ...
		    if(results[0].count === 0){
				...
				var sql = "UPDATE oauth_clients set client_name=?,client_details=?,group_policy_id=? WHERE client_id=?";
	    	        	//var values = [[client_name, client_details, redirect_uri, scope, group_policy_id, client_id],];
				connection.query(sql, [client_name, client_details, group_policy_id, client_id], function (error, results, fields) {
					if (error) {
                    				result = {'status':'fail','result': error.message};
                    				return res.json(result);
					}else{
