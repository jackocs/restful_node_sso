...
router.get('/:attribute', function (req, res) {
	...
	child = exec("docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' db ", function (error, stdout, stderr) {
        ...
        connection.connect(function(err) {                                                                                                                      
            ...
            connection.query("select * from oauth_objectclasses where attribute='"+ attribute +"'", function (error, results, fields) {
                if (error) {                                                                                          
...
///                    return res.json(result);                                                                                                                    
            			...
            			connection.query("DELETE FROM oauth_objectclasses where attribute='"+ attribute +"'", function (error, results, fields) {
                			if (error) {	
						let result = {'status':'fail','result': error.message};
						return res.json(result); 
					}else{
						exec("php /home/xIDM-SSO-Cent8/sso/idp/config/mysql2redis_local.php oauth_objectclasses delete "+ attribute, function (error, stdout, stderr) {
                                                        if (error !== null) {
                                                                result = {'status':'fail','result': error};
                                                                return res.json(result);
                                                        }else{
