...
router.post('', function(req, res) {
    if (req.body.group_policy_name === undefined || req.body.group_policy_name === null || 
	...
	config.host = stdout.trim();
	    ...
	    connection.query("select COUNT(*) as count from oauth_group_policy where group_policy_name='"+ group_policy_name +"'", function (error, results, fields) {                                                      
		if (error) {
		    ...
		    if(results[0].count > 0){
					...
					if (error) {
						...
						exec("php /home/xIDM-SSO-Cent8/sso/idp/config/mysql2redis_local.php oauth_group_policy add "+ group_policy_name, function (error, stdout, stderr) {
                                                        if (error !== null) {
                                                                result = {'status':'fail','result': error};
                                                                return res.json(result);
                                                        }else{
